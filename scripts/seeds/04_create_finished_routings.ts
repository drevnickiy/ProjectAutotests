...
interface FinishedRoutingConfig {
  id: string;
  name: string;
  productName: string;
  productUrl?: string;
...
test.describe('04. Створення техкарт для готової продукції (Finished Routings)', () => {
  let loginPage: LoginPage;
  let routingPage: GenProductionRoutingPage;
  const dataPath = path.resolve(__dirname, '../data/finished_routings.json');
  const routings: FinishedRoutingConfig[] = JSON.parse(fs.readFileSync(dataPath, 'utf-8'));
  ...
  for (const config of routings) {
    test(`Створення техкарти готової продукції: ${config.name}`, async ({ page }) => {
