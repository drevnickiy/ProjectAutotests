...
interface SemiFinishedProductData {
  code: string;
  name: string;
  category: string;
  unit: string;
...
test.describe('01. Створення напівфабрикатів (Semi-finished Products)', () => {
  ...
  const dataPath = path.resolve(__dirname, '../data/semi_finished_products.json');
  const products: SemiFinishedProductData[] = JSON.parse(fs.readFileSync(dataPath, 'utf-8'));
  ...
  for (const prod of products) {
    test(`Створення напівфабрикату: ${prod.name}`, async ({ page }) => {
      test.setTimeout(180000);
      console.log(`\n======================================================`);
      console.log(`📦 Створення напівфабрикату [${prod.code}] "${prod.name}"...`);
      console.log(`======================================================`);
      ...
      console.log(`✅ Напівфабрикат "${prod.name}" успішно створено з типом контролю партії "${prod.batchControl || 'FEFO'}"!`);
