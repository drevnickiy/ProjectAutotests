...
test.describe('Полная валидация раздела Продукты и типов карточек (Готовий продукт, Відходи, Напівфабрикат)', () => {
  let loginPage: LoginPage;
  let productsListPage: ProductsPage;
  let productsCategoriesPage: ProductsCategoriesPage;

  test.beforeEach(async ({ page }) => {
    test.setTimeout(180000); // 3 минуты на подробную валидацию всех 9 вкладок карточки
    loginPage = new LoginPage(page);
    await loginPage.open('https://xlab-analyst-main.poligon.crmgenesis.com/0/Shell/#Section/Products_ListPage');
    await loginPage.login();

    productsListPage = new ProductsPage(page);
    productsCategoriesPage = new ProductsCategoriesPage(page);
  });

  test('1. Проверка колонок реестра Продукты (Products_ListPage)', async ({}, testInfo) => {
    await productsListPage.openProductsListPage();
    const listFieldsData = JSON.parse(
      fs.readFileSync(path.resolve(__dirname, '../src/locators/Продукти.json'), 'utf-8')
    );
    await productsListPage.validateFieldsFromDict('Products_ListPage', listFieldsData.registryColumns, testInfo);
  });

  test('2. Проверка полей модального окна добавления Продукта (Products_MiniPage/add)', async ({}, testInfo) => {
    await productsListPage.openProductsAddModal();
    const miniPageData = JSON.parse(
      fs.readFileSync(path.resolve(__dirname, '../src/locators/Продукти_МініСторінка.json'), 'utf-8')
    );
    await productsListPage.validateFieldsFromDict('Products_MiniPage_Modal', miniPageData.modalFormFields, testInfo);
  });

  test('3. Проверка карточки "Готовий продукт" (левая панель и все вкладки)', async ({}, testInfo) => {
  ...
  test('4. Проверка карточки "Відходи" (левая панель и все вкладки)', async ({}, testInfo) => {
  ...
  test('5. Проверка карточки "Напівфабрикат" (левая панель и ВСЕ вкладки от БАЛАНС до ЦІНИ І ЗАЛИШКИ)', async ({}, testInfo) => {
