...
test('Створення 2 нових напівфабрикатів (Категорія, Тип контролю партії FEFO, Термін придатності)', async ({ page }) => {
  test.setTimeout(300000); // 5 хвилин
  const loginPage = new LoginPage(page);

  const semiFinishedList = [
    {
      name: 'НФ-501 Емульсійна база універсальна',
      category: 'Напівфабрикат',
      shelfLifeDays: '730',
      batchControl: 'FEFO'
    },
    {
      name: 'НФ-502 Гель-основа зволожуюча',
      category: 'Напівфабрикат',
      shelfLifeDays: '730',
      batchControl: 'FEFO'
    }
  ...
  for (let i = 0; i < semiFinishedList.length; i++) {
    const item = semiFinishedList[i];
    console.log(`\n================================================================================`);
    console.log(`🧪 [${i + 1}/${semiFinishedList.length}] СТВОРЕННЯ НАПІВФАБРИКАТУ: "${item.name}"`);
    console.log(`================================================================================`);
    ...
    // 3. Категорія (ліва панель: Напівфабрикат)
    ...
    console.log(`✅ [${i + 1}/${semiFinishedList.length}] Напівфабрикат "${item.name}" успішно створено та збережено!`);
  }

  console.log('\n🎉 Всі 2 напівфабрикати успішно збережено з Категорією "Напівфабрикат", FEFO та терміном придатності!');
});
