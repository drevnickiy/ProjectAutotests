...
test.describe('TC-PLAN-01-NEG: Валідація затвердження ПГП без Технологічної карти', () => {
  ...
  test('Спроба затвердження продукту "Шампунь 2" без ТК викликає помилку / поп-ап блокування', async ({ page }) => {
    ...
    // 2. Додавання продукту у деталь "План потреби готової продукції"
    console.log('📌 5. Додавання продукту у деталь "План потреби готової продукції"...');
    ...
    // Автоматично скролимо випадаючий список вниз за допомогою колеса миші до знаходження "Шампунь 2 без ТК"
    console.log('   🔍 Прокручуємо випадаючий список вниз у пошуках "Шампунь 2 без ТК"...');
    ...
    const targetOption = page.locator('.cdk-overlay-pane mat-option, [role="listbox"] [role="option"]')
      .filter({ hasText: 'Шампунь 2 без ТК' })
      .first();
    ...
    console.log(`   ✅ Знайдено точний збіг: "${prodName}"`);
    ...
    // Підтвердження системного діалогу, якщо з'явиться
    ...
    // 3. Спроба переведення у статус "Затверджено" через усі стадії DCM
    console.log('📌 6. Спроба переведення ПГП у статус "Затверджено"...');
    ...
    const errorDialog = page.getByText(/Не всі продукти мають технологічну карту\.?\s*Заповніть технологічні карти для всіх/i)
      .or(page.locator('mat-dialog-container, crt-modal, .ts-messagebox, .crt-toast, [role="dialog"], [role="alertdialog"]').filter({ hasText: /Не всі продукти мають технологічну карту/i }))
      .first();
    ...
    // Перевірка, що статус НЕ перейшов у Затверджено (залишився активним початковий або поп-ап блокує)
    console.log('🎉 Успішно! Негативний тест перевірки відсутності ТК пройдено.');
  });
});
