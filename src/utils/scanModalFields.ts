...
async function scanModalFields() {
  ...
  if (isLoginPage) {
    ...
    console.log('[3] Авторизованы. Переход на страницу...');
    await page.goto(targetUrl, { waitUntil: 'domcontentloaded' });
  ...
  // Кликом по точным координатам или элементу синей кнопки "+ Додати" в шапке раздела
  console.log('[5] Ищем синюю кнопку "+ Додати" реестра...');
  
  const clicked = await page.evaluate(() => {
    // Ищем синюю кнопку раздела
    ...
    const sectionAddBtn = allButtons.find(b => {
      ...
      // Синяя кнопка раздела расположена сверху справа (x > 1000, y > 40 && y < 150)
  ...
  if (!clicked) {
    console.log('Попытка клика через Playwright locator...');
    await page.locator('.crt-button--contained').filter({ hasText: 'Додати' }).last().click();
  }

  console.log('[6] Ожидаем открытие карточки / поп-апа...');
  await page.waitForTimeout(6000);

  // Скриншот открывшейся формы
  await page.screenshot({ path: 'modal_popup.png', fullPage: true });

  console.log('[7] Извлекаем локаторы всех полей формы/модального окна...');

  const fieldsData = await page.evaluate(() => {
    const result: { [key: string]: string } = {};

    // Ищем инпуты, комбобоксы, выпадающие списки и поля ввода
    const inputs = Array.from(
      document.querySelectorAll('input:not([type="hidden"]), textarea, select, mat-select, [role="combobox"], crt-combobox, crt-input')
    ) as HTMLElement[];
    ...
    inputs.forEach((el, index) => {
      ...
      // Label по for
      if (el.id) {
        const lbl = document.querySelector(`label[for="${CSS.escape(el.id)}"]`) as HTMLElement;
        if (lbl && lbl.innerText.trim()) fieldName = lbl.innerText.trim();
      }

      // Родительский компонент полей Creatio
      if (!fieldName) {
    ...
    // Также соберем названия полей Creatio (crt-field label)
    const crtLabels = Array.from(document.querySelectorAll('crt-label, label.crt-label, .crt-field-title, .mat-mdc-floating-label')) as HTMLElement[];
  ...
  console.log('\n--- 📋 Собраны поля открывшейся карточки / поп-апа ---');
  console.log(JSON.stringify(fieldsData, null, 2));

  fs.writeFileSync('src/locators/План_виробництва_Модалка.json', JSON.stringify(fieldsData, null, 2), 'utf-8');
  console.log('\nРезультаты сохранены в: План_виробництва_Модалка.json');

  await browser.close();
}

scanModalFields().catch((err) => {
